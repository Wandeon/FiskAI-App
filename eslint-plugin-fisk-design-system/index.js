  ...
  rules: {
    "no-hardcoded-colors": {
      meta: {
        type: "problem",
        docs: {
          description: "Prevent hardcoded Tailwind color usage in favor of semantic design tokens",
      ...
      create(context) {
            ...
            if (matches) {
              ...
              if (!suggestion) {
                ...
                if (hardcodedColor.startsWith("text-")) {
                  ...
                  suggestion =
                    "bg-surface, bg-surface-1, bg-surface-2, or status colors (bg-success-bg, etc.)"
        ...
        return {
          ...
          CallExpression(node) {
            if (node.callee && (node.callee.name === "cn" || node.callee.name === "clsx")) {
