...
test.describe("Invoice Creation", () => {
  ...
  test.describe("Authenticated - Invoice Form", () => {
    ...
    test("should have buyer selection field", async ({ page }) => {
      ...
      const buyerField = page
        .getByRole("combobox", { name: /kupac|buyer|kontakt|contact/i })
    ...
    test("should allow adding line items", async ({ page }) => {
      ...
      const addItemButton = page
        .getByRole("button", { name: /dodaj|add|stavk|item/i })
    ...
    test("should show total calculation", async ({ page }) => {
      ...
      await expect(page.getByText(/ukupno|total|iznos/i).first()).toBeVisible({ timeout: 10000 })
    })
    ...
    test("should have submit/save button", async ({ page }) => {
      ...
      const submitButton = page.getByRole("button", {
        name: /spremi|save|kreiraj|create|izdaj|issue/i,
      })
    ...
    test("should navigate back to invoice list", async ({ page }) => {
      ...
      const backButton = page
        .getByRole("button", { name: /natrag|back|povratak/i })
    ...
    test("should validate required fields on submit", async ({ page }) => {
      ...
      const submitButton = page
        .getByRole("button", { name: /spremi|save|kreiraj|create|izdaj|issue/i })
        .first()
      ...
      if (await submitButton.isVisible()) {
        ...
        await expect(
          page
            .getByText(/obavez|required|popun|fill/i)
            .first()
  ...
  test.describe("E-Invoice Specific", () => {
    ...
    test("should display e-invoice creation form", async ({ page }) => {
      ...
      await expect(
        page
          .getByRole("heading", { name: /e-račun|e-invoice|novi/i })
          .first()
...
test.describe("Invoice List", () => {
  ...
  test.describe("Authenticated", () => {
    ...
    test("should display invoice list page", async ({ page }) => {
      ...
      await expect(
        page
          .getByRole("heading", { name: /račun|invoice/i })
          .first()
    ...
    test("should have create new invoice button", async ({ page }) => {
      ...
      const newButton = page
        .getByRole("link", { name: /novi|new|kreiraj|create/i })
