...
describe("PO-SD threshold validation", () => {
  ...
  describe("60,000 EUR pausalni threshold", () => {
    it("should pass validation when income is below 60,000 EUR", () => {
      ...
      expect(
        result.errors.some((e) => e.includes("prelazi granicu za pausalno oporezivanje"))
      ).toBe(false)
    })
    ...
    it("should include proper Croatian message with diacritics", () => {
      ...
      expect(
        result.errors.some(
          (e) =>
            e.includes("Morate prijeci na obrt na dohodak") && e.includes("registrirati se za PDV")
        )
      ).toBe(true)
    })
    ...
    it("should display both income and threshold values in error", () => {
      ...
      const thresholdError = result.errors.find((e) =>
        e.includes("prelazi granicu za pausalno oporezivanje")
      )
