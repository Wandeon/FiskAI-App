...
function getInvoiceStatusColor(status: EInvoiceStatus): UnifiedDocument['statusColor'] {
  switch (status) {
    ...
    case 'SENT':
    case 'PENDING_FISCALIZATION': return 'blue'
    case 'FISCALIZED':
    case 'DELIVERED':
...
export async function queryUnifiedDocuments(options: UnifiedQueryOptions): Promise<UnifiedQueryResult> {
  ...
  const normalizedExpenses: UnifiedDocument[] = expenses.map(exp => ({
    ...
    statusColor: exp.status === 'APPROVED' || exp.status === 'PAID' ? 'green' : exp.status === 'REJECTED' ? 'red' : 'gray',
