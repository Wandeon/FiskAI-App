...
export function validateEN16931Compliance(invoice: EN16931Invoice): ComplianceResult {
  ...
  if (invoice.netAmount !== undefined && 
    ...
    const calculatedTotal = Number((Number(invoice.netAmount) + Number(invoice.vatAmount)).toFixed(2));
    const actualTotal = Number(invoice.totalAmount);
    
    if (Math.abs(calculatedTotal - actualTotal) > 0.01) { // Allow small rounding differences
      errors.push(`Total amount (${actualTotal}) doesn't match net amount + VAT (${calculatedTotal})`);
...
export function getComplianceSummary(invoice: EN16931Invoice): {
  ...
  const criticalErrors = [
    ...
    ...croatianResult.errors.filter(error => 
      error.includes("JIR") || 
      error.includes("ZKI") || 
