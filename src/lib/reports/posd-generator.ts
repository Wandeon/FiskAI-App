 ...
 */
export async function generatePOSD(
  companyId: string,
  year: number
): Promise<POSDData> {
  try {
    ...
    const socialContributions = {
      ...
      totalContributions: estimatedIncome * (
 ...
 */
export async function generatePOSDForePorezna(
  companyId: string,
  year: number
): Promise<string> {
  try {
...
export async function generateQuarterlyPOSDSummary(
  ...
  try {
    ...
    for (let quarter = 1; quarter <= 4; quarter++) {
      ...
      const quarterlyExpenses = await db.expense.findMany({
        where: {
          companyId,
          date: {
            gte: startDate,
      ...
      const quarterlyIncome = quarterlyInvoices.reduce((sum, inv) => sum + Number(inv.totalAmount), 0);
      const quarterlyExpenses = quarterlyExpenses.reduce((sum, exp) => sum + Number(exp.totalAmount), 0);
      const quarterlyTaxableIncome = Math.max(0, quarterlyIncome - quarterlyExpenses);
      
      // Calculate estimated tax for the quarter
      const quarterlyTax = calculateTaxForIncome(quarterlyTaxableIncome);
      const quarterlySocialContrib = quarterlyIncome * (
        SOCIAL_CONTRIBUTION_RATES.pension +
        SOCIAL_CONTRIBUTION_RATES.disability + 
        SOCIAL_CONTRIBUTION_RATES.health +
        SOCIAL_CONTRIBUTION_RATES.unemployment
      ...
      quarters.push({
        ...
        expenses: Number(quarterlyExpenses.toFixed(2)),
