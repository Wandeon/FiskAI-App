...
export async function POST(request: NextRequest) {
  try {
    ...
    if (name) {
      ...
      if (!status[name]) {
        return NextResponse.json({ success: false, message: "Circuit breaker not found", reset: [] }, { status: 404 })
      }
      if (status[name].state === "closed") {
        return NextResponse.json({ success: false, message: "Circuit breaker is already closed", reset: [] }, { status: 400 })
      }
      ...
      console.log("[workers-circuit-breaker-reset] Manual reset for all breakers (" + resetBreakers.length + ") by admin: " + user.email)
