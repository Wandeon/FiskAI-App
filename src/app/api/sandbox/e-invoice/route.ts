...
const sandboxTestSchema = z.object({
  ...
  invoiceData: z.object({
    ...
    buyerOib: z.string().length(11).regex(/^\d{11}$/, "OIB must be 11 digits"),
    sellerOib: z.string().length(11).regex(/^\d{11}$/, "OIB must be 11 digits"),
  }).optional(),
  ...
  try {
    ...
    switch (type) {
      ...
      case "status":
        if (!jir) {
          return NextResponse.json(
            { error: "JIR is required for status test" },
            { status: 400 }
          );
        }
      ...
      case "cancel":
        if (!jir) {
          return NextResponse.json(
            { error: "JIR is required for cancel test" },
            { status: 400 }
          );
        }
...
async function testCancel(company: any, jir: string) {
  try {
    ...
    return NextResponse.json({
      ...
      testType: "cancel",
      result,
      message: result.success ? `Cancellation test passed for JIR: ${jir}` : `Cancellation test failed for JIR: ${jir}`,
    });
