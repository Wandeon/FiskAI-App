...
async function processPdfOrImage(filePath: string, ext: string) {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: 'system', content: BANK_STATEMENT_SYSTEM_PROMPT },
            {
              role: 'user',
              content: [
                { type: 'text', text: 'Extract transactions from this bank statement image. Return valid JSON only.' },
  ...
  if (openingBalance !== null && closingBalance !== null) {
    const calcClosing = openingBalance + transactions.reduce((sum: number, t: any) =>
      sum + (t.direction === 'INCOMING' ? t.amount : -t.amount), 0)
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: 'system', content: INVOICE_SYSTEM_PROMPT },
            {
              role: 'user',
              content: [
                { type: 'text', text: 'Extract invoice data from this image. Return valid JSON only.' },
                { type: 'image_url', image_url: { url: `data:${mimeType};base64,${imageBase64}` } },
              ],
            },
          ],
