...
export function NewsTableClient({ initialPosts, categories }: NewsTableClientProps) {
  ...
  const handleFilterChange = (type: "status" | "category" | "impact", value: string) => {
    setCurrentPage(1)
    switch (type) {
      case "status":
            ...
            paginatedPosts.map((post) => (
              <tr key={post.id} className="hover:bg-[var(--surface-secondary)]/50">
                ...
                <td className="px-4 py-3 text-[var(--muted)]">
                  {categories.find((c) => c.id === post.categoryId)?.nameHr ||
                    post.categoryId ||
                    "-"}
                </td>
                <td className="px-4 py-3">
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    pending: "bg-surface/20 text-secondary ",
    draft: "bg-warning/20 text-warning-text ",
    reviewing: "bg-interactive/20 text-info-text dark:text-info-text",
    published: "bg-success/20 text-success-text ",
  }
