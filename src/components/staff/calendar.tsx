...
function getSeverityIcon(daysLeft: number, severity: string | null) {
  if (daysLeft <= 0) return <AlertCircle className="h-5 w-5 text-danger-icon" />
  if (daysLeft <= 3 || severity === "critical")
    return <AlertTriangle className="h-5 w-5 text-danger-icon" />
  if (daysLeft <= 7 || severity === "high")
    return <AlertTriangle className="h-5 w-5 text-warning-icon" />
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Upcoming deadlines across all assigned clients</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-danger-icon">{criticalCount}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-warning-icon">{thisWeekCount}</div>
        ...
        <CardContent>
          {deadlines.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <CheckCircle2 className="h-12 w-12 text-success-icon mb-4" />
                ...
                return (
                  <div key={date} className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      {isTomorrow && <Badge variant="warning">Tomorrow</Badge>}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="flex flex-col items-end gap-2">
                                  <Badge variant={severity}>{deadline.deadlineType}</Badge>
