...
async function getStaffStats(userId: string) {
  ...
  const [assignedClients, pendingTickets, complianceDeadlines, draftInvoices, pendingExpenses] = await Promise.all([
  ...
  const relevantDeadlines = complianceDeadlines.filter(deadline => {
    ...
    return companies.some(company => {
      ...
      if (deadline.deadline_type.toLowerCase().includes('vat') ||
...
async function getUpcomingDeadlineDetails(userId: string, limit: number = 5): Promise<DeadlineDetail[]> {
  // Get assigned company IDs
  const assignments = await db.staffAssignment.findMany({
    where: { staffId: userId },
    select: { companyId: true },
...
function getActivityIcon(type: string) {
  switch (type) {
    ...
    case 'expense': return <Receipt className="h-4 w-4 text-warning-text" />
    case 'ticket': return <MessageSquare className="h-4 w-4 text-accent" />
    case 'document': return <Upload className="h-4 w-4 text-info" />
    case 'audit': return <ActivityIcon className="h-4 w-4 text-muted-foreground" />
          ...
          <CardContent>
            <div className="space-y-3">
              {upcomingDeadlines.map((deadline) => {
                const urgencyColor =
                  deadline.daysUntil <= 3 ? 'text-danger-text' :
                  deadline.daysUntil <= 7 ? 'text-warning-text' :
                  'text-info'

                const urgencyBg =
                  deadline.daysUntil <= 3 ? 'bg-danger-bg' :
                  deadline.daysUntil <= 7 ? 'bg-warning-bg' :
                  'bg-muted'

                return (
                  <div key={deadline.id} className="flex items-center justify-between gap-4 p-3 rounded-lg border">
                    <div className="flex items-center gap-3 flex-1">
                      <div className={`flex items-center justify-center w-12 h-12 rounded-full ${urgencyBg}`}>
