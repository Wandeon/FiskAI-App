...
export function ComplianceBadge({ variant = "footer", showDetails = false }: ComplianceBadgeProps) {
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            <Shield className="h-3.5 w-3.5 text-success-text" aria-hidden="true" />
  ...
  if (variant === "card") {
    return (
      <div
        className="flex items-center gap-2 p-3 border rounded-lg bg-success-bg border-success-border"
        role="status"
        aria-label={statusLabel}
      >
