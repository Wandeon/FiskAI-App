...
### 2. Host header poisoning via `AUTH_TRUST_HOST=true` (High)
...
- **Remediation:** Encrypt at rest (KMS/lib­sodium), never echo the raw key back, and rotate any existing keys.
...
3. Remove `AUTH_TRUST_HOST` or network-isolate the app.
