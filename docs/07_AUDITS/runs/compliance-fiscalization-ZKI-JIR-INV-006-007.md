...
- **Observation** – No explicit database constraint prevents direct DB edits after JIR; protection relies on application-layer status checks. Consider adding DB-level immutability (trigger or row-level policy) for stronger enforcement. *(Gap)*
